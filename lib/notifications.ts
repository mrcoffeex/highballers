...
export async function scheduleGameReminder(eventId: string, title: string, dateTime: string) {
...
export async function cancelGameReminder(eventId: string) {
  await Notifications.cancelScheduledNotificationAsync(`game-reminder-${eventId}`).catch(() => undefined);
}
